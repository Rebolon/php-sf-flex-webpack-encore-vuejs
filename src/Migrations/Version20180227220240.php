 ...
 */
class Version20180227220240 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TABLE job (id INTEGER NOT NULL, translation_key VARCHAR(256) NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO job (id, translation_key) SELECT id, tanslation_key FROM __temp__job');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__project_book_creation AS SELECT id, book_id, author_id, job_id FROM project_book_creation');
        ...
        $this->addSql('INSERT INTO project_book_creation (id, book_id, author_id, job_id) SELECT id, book_id, author_id, job_id FROM __temp__project_book_creation');
        ...
        $this->addSql('CREATE TABLE review (id INTEGER NOT NULL, book_id INTEGER DEFAULT NULL, body CLOB DEFAULT NULL COLLATE BINARY, username VARCHAR(512) DEFAULT NULL COLLATE BINARY, publication_date DATETIME DEFAULT \'now()\' NOT NULL, rating INTEGER NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_794381C616A2B381 FOREIGN KEY (book_id) REFERENCES book (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('DROP TABLE __temp__book');
        $this->addSql('CREATE INDEX IDX_CBE5A331D94388BD ON book (serie_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__job AS SELECT id, translation_key FROM job');
        $this->addSql('DROP TABLE job');
        $this->addSql('CREATE TABLE job (id INTEGER NOT NULL, tanslation_key VARCHAR(256) NOT NULL COLLATE BINARY, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO job (id, tanslation_key) SELECT id, translation_key FROM __temp__job');
        $this->addSql('DROP TABLE __temp__job');
        $this->addSql('DROP INDEX IDX_9A80042BBE04EA9');
        $this->addSql('DROP INDEX IDX_9A80042B16A2B381');
        $this->addSql('DROP INDEX IDX_9A80042BF675F31B');
        ...
        $this->addSql('DROP TABLE project_book_creation');
        $this->addSql('CREATE TABLE project_book_creation (id INTEGER NOT NULL, job_id INTEGER DEFAULT NULL, book_id INTEGER DEFAULT NULL, author_id INTEGER DEFAULT NULL, PRIMARY KEY(id))');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__review AS SELECT id, book_id, rating, body, username, publication_date FROM review');
        $this->addSql('DROP TABLE review');
        $this->addSql('CREATE TABLE review (id INTEGER NOT NULL, book_id INTEGER DEFAULT NULL, body CLOB DEFAULT NULL, username VARCHAR(512) DEFAULT NULL, publication_date DATETIME DEFAULT \'now()\' NOT NULL, rating INTEGER DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO review (id, book_id, rating, body, username, publication_date) SELECT id, book_id, rating, body, username, publication_date FROM __temp__review');
        $this->addSql('DROP TABLE __temp__review');
        $this->addSql('CREATE INDEX IDX_794381C616A2B381 ON review (book_id)');
    }
